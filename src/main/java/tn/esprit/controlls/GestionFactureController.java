...
public class GestionFactureController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("tn/esprit/GestionFacture.fxml"));
        ...
        FactureCardLoader = new FXMLLoader(getClass().getResource("/tn/esprit/FactureCard.fxml"));
        loadFactureData();
    ...
    private void loadFactureData() {
        ...
        for (Facture facture : factures) {
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("/tn/esprit/FactureCard.fxml"));
    ...
    void Imprimez(ActionEvent event) {
        if (selectedFacture != null) {
            ...
            int remise = selectedFacture.getRemise() ;
            ...
            imprimezFacture("facture.pdf", nomClient, date, remise,prixTotal, tva, prixTTC);
    ...
    private void imprimezFacture(String nomFichier, String nomClient, String date, int remise, String prixTotal, String tva, String prixTTC) {
        ...
        try {
            ...
            // Ajouter le logo
            PDImageXObject logo = PDImageXObject.createFromFile("src/main/resources/tn/esprit/logo.png", document);

            // Ajouter le logo en haut à droite
            contentStream.drawImage(logo, page.getMediaBox().getWidth() - 150, page.getMediaBox().getHeight() - 50, 100, 40);


            // Ajouter le nom du groupe au centre de la page
            float textWidth = font.getStringWidth("Poneglyphe-S-Hunters") / 1000 * fontSize;
            float middleX = (page.getMediaBox().getWidth() - textWidth) / 2;
            float topY = page.getMediaBox().getHeight() - 50;

            contentStream.beginText();
            contentStream.setFont(font, fontSize);
            contentStream.setLeading(14.5f);
            contentStream.newLineAtOffset(middleX,  topY);
            contentStream.showText("Poneglyphe-S-Hunters");
            contentStream.endText();
            // Ajouter la signature en bas de la page
            PDImageXObject signature = PDImageXObject.createFromFile("src/main/resources/tn/esprit/signature.png", document);
            contentStream.drawImage(signature, 300, 100, 100, 50);


            // Ajouter le cadre
            contentStream.addRect(70, 70, page.getMediaBox().getWidth() - 140, page.getMediaBox().getHeight() - 140);
            contentStream.stroke();
            // Ajouter le contenu de la facture
            contentStream.beginText();
            contentStream.setFont(font, fontSize);
            contentStream.newLineAtOffset(100, 600); // Ajuster la position verticale
    ...
    private void Retour(ActionEvent event) {
        try {
            Parent root = FXMLLoader.load(getClass().getClassLoader().getResource("tn/esprit/Menu.fxml"));
