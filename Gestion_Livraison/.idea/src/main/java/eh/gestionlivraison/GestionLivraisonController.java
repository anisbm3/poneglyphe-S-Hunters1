...
public class GestionLivraisonController implements Initializable {
    ...
    void onLivraisonSelected(MouseEvent event) {
        ...
        Livraison selectedLivraison = (Livraison) selectedCard.getUserData();
    ...
    public void EditLivraison() {
        if (selectedLivraison != null) {
            ...
            // Afficher un message d'avertissement si aucune livraison n'est sélectionnée
            showAlert(Alert.AlertType.WARNING, "Sélectionnez une livraison", "Veuillez sélectionner une livraison à éditer.");
        }
    }
    ...
    void DeleteLivraisonSelected(ActionEvent event) {
        for (Livraison livraison : getLivraisons()) {
            if (livraison.isSelected()) {
                boolean success = sl.delete(livraison);
                if (success) {
                    showAlert(Alert.AlertType.INFORMATION, "Succès", "Livraison supprimée !");
                } else {
                    showAlert(Alert.AlertType.ERROR, "Erreur", "Erreur lors de la suppression de la livraison.");
                }
                livraison.setSelected(false); // Déselectionner la livraison après la suppression
            }
        }
